...
export const ShareDelights = () => {
  ...
  useEffect(() => {
    const fetchDelights = async () => {
      ...
      try {
        const { data: delights, error } = await centralSupabase
          .from('delights')
          .select('*')
          .eq('user_id', user.id)
  ...
  const handleShareToCommunity = async (delight: Delight) => {
    ...
    try {
      ...
      try {
        ...
        const tableReady = await checkCommunityDelightsTable();
        
        if (!tableReady) {
          toast({ 
            title: "System Error", 
            description: "Could not prepare the community feature. Please try again later.", 
            variant: "destructive" 
          });
          return;
        }
      } catch (tableError) {
        console.error("Error checking/creating community table:", tableError);
      }
      
      // Now try to insert the delight
      const { error } = await centralSupabase
      ...
      if (error) {
        ...
        if (error.code === '42P01' || error.message.includes('does not exist')) {
        ...
        } else if (error.code === '406' || error.code === 'PGRST116') {
          // Handle Not Acceptable error (406)
          console.log("Received 406 Not Acceptable error, trying alternative approach");
          ...
          const response = await fetch(`${supabaseUrl}/rest/v1/community_delights`, {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            // Success with direct fetch approach
            toast({ 
            ...
            console.error("Error with direct API call:", errorData);
            toast({ 
              title: "Error", 
              description: `Could not share to community (${response.status})`, 
              variant: "destructive" 
            });
          }
        } else {
