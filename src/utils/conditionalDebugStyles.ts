...
const isDebugMode = (): boolean => {
  // Only enable by explicit URL parameter to reduce overhead by default
  const urlParams = new URLSearchParams(window.location.search);
  if (urlParams.has('debug') || urlParams.has('debugStyles')) {
    return true;
  }
  
  // Disable localStorage check to avoid performance issues
  return false;
};

// Function to conditionally load the DisableAnimations CSS
export const loadDebugStyles = (): void => {
  if (isDebugMode()) {
...
const addDebugControls = (): void => {
  ...
  toggleButton.addEventListener('click', () => {
    ...
    if (link) {
      (link as HTMLLinkElement).disabled = !(link as HTMLLinkElement).disabled;
      toggleButton.innerText = (link as HTMLLinkElement).disabled ? 'Enable Debug Styles' : 'Disable Debug Styles';
      
      // Flash the background to indicate the change
      document.body.style.transition = 'background-color 0.3s';
      document.body.style.backgroundColor = (link as HTMLLinkElement).disabled ? '#f0fff0' : '#fff0f0';
      setTimeout(() => {
