  ...
  return (
    <div className="flex flex-col items-start gap-5 w-full">
      <div className="flex flex-nowrap gap-2.5 w-full overflow-y-auto overflow-x-auto max-w-full">
        {/* First banner with max-width 200px - Enhanced mobile responsiveness */}
        <div className="flex-shrink-0 w-full sm:w-auto max-w-[200px] bg-gradient-to-b from-[#49DADD] to-[rgba(195.50,253.79,255,0.20)] p-3 sm:p-2.5 rounded-[10px] flex flex-col justify-between shadow-[1px_2px_4px_rgba(73,218,234,0.5)]">
          <p className="text-[#148BAF] font-happy-monkey lowercase mb-3 sm:mb-0 text-sm sm:text-xs">
            get your free session with our best of the therapist
          </p>
          <div className="mt-auto">
            <Link to="/therapist-listing">
              <button className="w-full py-3 sm:py-2.5 bg-[#148BAF] text-white rounded-[10px] font-happy-monkey lowercase text-sm sm:text-xs min-h-[44px] sm:min-h-auto touch-manipulation hover:bg-[#0f7a99] active:scale-95 transition-all">
                discover therapist
              </button>
            </Link>
          </div>
        ...
        {practitioners.slice(0, 4).map((practitioner) => (
          <div 
            key={practitioner.id} 
            className="flex-shrink-0 w-full sm:w-[calc(50%-5px)] md:w-[calc(33.33%-7px)] lg:w-[calc(25%-7.5px)] bg-white p-3 sm:p-2.5 rounded-[20px] border border-[rgba(4,196,213,0.3)] shadow-[1px_2px_4px_rgba(73,218,234,0.5)] hover:shadow-lg transition-all cursor-pointer"
          >
            <div className="h-[100px] bg-gray-200 relative rounded-[4px] mb-2.5">
              {practitioner.badge && (
                <div className="absolute top-2.5 left-2.5 bg-black px-2 sm:px-1.5 py-2 sm:py-1.5 rounded-[10px] min-h-[32px] sm:min-h-auto flex items-center justify-center">
                  <span className="text-white text-xs font-happy-monkey lowercase">{practitioner.badge}</span>
                </div>
              )}
            </div>
            <div className="text-base font-happy-monkey text-black lowercase mb-1 line-clamp-2">{practitioner.name}</div>
            <div className="text-xs text-[#148BAF] font-happy-monkey lowercase mb-2 sm:mb-1 line-clamp-2">{practitioner.specialty}</div>
            
            <div className="flex items-center gap-1 sm:gap-0.5 mb-2 sm:mb-1">
              ...
              <span className="flex-1 text-xs text-[#148BAF] font-happy-monkey lowercase line-clamp-1">masters in psychology</span>
            </div>
            
            <div className="flex justify-between items-center flex-wrap gap-2">
              <div className="flex items-center gap-1 sm:gap-0.5 flex-shrink-0">
                <svg width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
                ...
                <span className="text-xs text-[#148BAF] font-happy-monkey lowercase line-clamp-1">{practitioner.rating} ({practitioner.reviews} reviews)</span>
              </div>
              <span className="text-sm text-black font-happy-monkey lowercase whitespace-nowrap">$ {practitioner.price}/session</span>
            </div>
          </div>
        ))}
      </div>
