...
const supabaseKey = process.env.VITE_SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_ANON_KEY || process.argv[3];

if (!supabaseUrl || !supabaseKey) {
  console.error(
    'Error: Missing Supabase credentials. Please provide them as environment variables or command line arguments.'
  );
  console.error('Usage: node fix-practitioners-rls.mjs <SUPABASE_URL> <SUPABASE_SERVICE_KEY>');
  process.exit(1);
}
...
async function applyRLSPolicies() {
  ...
  try {
    ...
    await supabase.rpc('pg_execute', {
      query: `
        DROP POLICY IF EXISTS "Allow public read access to practitioners" ON public.practitioners;
    ...
    await supabase.rpc('pg_execute', {
      query: `
        DROP POLICY IF EXISTS "Allow users to update their own practitioner profiles" ON public.practitioners;
        CREATE POLICY "Allow users to update their own practitioner profiles" 
        ON public.practitioners 
        FOR UPDATE 
    ...
    await supabase.rpc('pg_execute', {
      query: `
        DROP POLICY IF EXISTS "Allow users to delete their own practitioner profiles" ON public.practitioners;
        CREATE POLICY "Allow users to delete their own practitioner profiles" 
        ON public.practitioners 
        FOR DELETE 
        USING (auth.uid() = user_id);
      `
