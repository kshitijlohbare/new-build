// batch-update-wellness-practices.mjs
// Update wellness practices in batches to avoid any syntax issues
...
// Process all practices
async function processAllPractices() {
  // Batch 1: Daily Stretching/Yoga (update), Hydration Habit, Balanced Diet
  const batch1 = [
    {
      id: 17,
      icon: "yoga",
  ...
  // Batch 2: Sleep Hygiene, Morning Routine, Single-Tasking, Reading
  const batch2 = [
    {
      id: 20,
      icon: "sleep",
  ...
  // Batch 3: Progressive Muscle Relaxation, Nature Walks, Time Blocking
  const batch3 = [
    {
      id: 24,
      icon: "relax",
  ...
  // Batch 4: Weekly Review, Digital Detox, Blue Light Limitation, Stimulant Avoidance
  const batch4 = [
    {
      id: 27,
      icon: "review",
  ...
  for (const practice of batch1) {
    await processPractice(practice);
  }

  console.log("Processing batch 2...");
  for (const practice of batch2) {
    await processPractice(practice);
  }

  console.log("Processing batch 3...");
  for (const practice of batch3) {
    await processPractice(practice);
  }

  console.log("Processing batch 4...");
  for (const practice of batch4) {
    await processPractice(practice);
  }
}

// Run the script
console.log("Starting wellness practices update script in batches...");
processAllPractices()
  .then(() => {
  ...
  .then(({ data, error }) => {
    if (error) {
      ...
      // Log practice names and IDs for verification
      console.log("\nPractices in database:");
      data.sort((a, b) => a.id - b.id).forEach(p => {
        console.log(`${p.id}. ${p.name} (Daily: ${p.is_daily ? 'Yes' : 'No'}, System: ${p.is_system_practice ? 'Yes' : 'No'})`);
      });
    }
  })
  .catch(err => {
