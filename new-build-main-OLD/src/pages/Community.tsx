...
export default function Community() {
  ...
  useEffect(() => {
    ...
    const fetchCommunityDelights = async () => {
      try {
        // Always try to ensure the table exists first
        try {
          console.log('Ensuring community_delights table exists...');
          const { checkCommunityDelightsTable } = await import('../scripts/checkCommunityDelights');
          await checkCommunityDelightsTable();
        } catch (initError) {
          console.error("Failed to check/initialize community delights table:", initError);
        }
      
        // Now fetch the delights with error handling for 406
        try {
          ...
          if (error) {
            ...
            // If we get a 406 Not Acceptable error, try direct fetch
            if (error.code === '406' || error.code === 'PGRST116') {
              console.log("Received 406 Not Acceptable error, trying direct fetch approach");
              
              // Try using the Supabase REST API directly
              const supabaseUrl = process.env.REACT_APP_SUPABASE_URL || 'https://svnczxevigicuskppyfz.supabase.co';
              const supabaseKey = process.env.REACT_APP_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN2bmN6eGV2aWdpY3Vza3BweWZ6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDQwNDk1NDAsImV4cCI6MjA1OTYyNTU0MH0.00MNZRYjGKHTEFvF0enW-VCZ4qgDnXC4LeV8XsjGaEU';
              
              const response = await fetch(`${supabaseUrl}/rest/v1/community_delights?order=created_at.desc`, {
                method: 'GET',
                headers: {
              ...
              if (response.ok) {
                ...
                console.log("Successfully fetched delights via direct API:", directData.length);
                
                setCommunityDelights((directData || []).map((d: any) => ({
                  id: Number(d.id),
                  text: String(d.text),
            ...
            console.log("Successfully fetched community delights:", data.length);
  ...
  return (
    <div className="flex flex-col gap-6 p-2 md:p-5 items-center w-full min-h-screen bg-gradient-to-br from-[#F7FFFF] via-[#E6F9FA] to-[#F7F7FF]">
      {/* Add the CSS styles */}
      <style dangerouslySetInnerHTML={{ __html: animationStyles }} />
      
      {/* Header with animation */}
      <div 
        className="text-center text-[#148BAF] text-3xl font-happy-monkey lowercase w-full mb-2 tracking-wide"
        style={{ 
      ...
      <div className="max-w-5xl w-full mx-auto flex flex-col gap-6">
        ...
        <div className="max-w-4xl w-full mx-auto">
          ...
          <div className="flex flex-col gap-6">
            ...
            {activeTab === 'practice' && (
              <div>
                <div className="text-lg text-[#148BAF] font-happy-monkey mb-4 flex items-center">
                ...
                {practicePosts.length === 0 ? (
                  <div 
                    className="text-center bg-white p-10 rounded-[20px] border border-[rgba(4,196,213,0.3)] shadow-md"
                    style={{ animation: 'fadeIn 0.5s ease-out 0.3s both' }}
                  >
                    <div className="text-[#148BAF] font-happy-monkey text-lg mb-2">No practices shared yet</div>
                    <p className="text-black font-happy-monkey">Create and share your own practices from the Practices page!</p>
                  </div>
                ) : (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start mb-4">
                            <div className="flex items-start gap-4">
                              {originalPractice?.icon && (
                                <div className="flex-shrink-0 mt-1">
                                  ...
                                  >
                                    <span className="text-[#148BAF] text-xl">
                              ...
                              <div className="flex-grow">
                                <div className="flex items-center gap-2 mb-1">
                                  <h3 className="text-2xl font-bold text-[#148BAF] font-happy-monkey lowercase">
                                    {post.title}
                                  </h3>
                                </div>
                            ...
                            {originalPractice?.streak !== undefined && (
                              <div 
                                className="flex items-center space-x-1 rounded-lg px-3 py-1.5 shadow-sm" 
                                style={{ 
                              ...
                              >
                                <img 
                                  ...
                                  className="h-5 w-5" 
                                />
                                <span className="text-[#148BAF] font-happy-monkey font-bold">
            ...
            {activeTab === 'delight' && (
              <div>
                <div className="text-lg text-[#E6A514] font-happy-monkey mb-4 flex items-center">
                ...
                <div className="flex justify-center mb-4">
                  <button 
                    ...
                    className="bg-[rgba(230,165,20,0.1)] hover:bg-[rgba(230,165,20,0.2)] text-[#E6A514] font-happy-monkey py-2 px-4 rounded-full text-sm flex items-center gap-2 transition-all"
                  >
                ...
                {filteredDelights.length === 0 ? (
                  <div 
                    className="text-center bg-white p-10 rounded-[20px] border border-[rgba(230,165,20,0.3)] shadow-md"
                    style={{ animation: 'fadeIn 0.5s ease-out 0.3s both' }}
                  >
                    <div className="text-[#E6A514] font-happy-monkey text-lg mb-2">No delights shared yet</div>
                    <p className="text-black font-happy-monkey">Right-click on a delight on your homepage to share it with the community!</p>
                  </div>
                ) : (
