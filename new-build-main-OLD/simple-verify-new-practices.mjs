// simple-verify-new-practices.mjs
...
async function checkForNewPractices() {
  ...
  const newPracticeNames = ['Daily Journaling', 'Screen Time Limitation', 'Daily Stretching/Yoga'];
  ...
  for (const practiceName of newPracticeNames) {
    ...
    if (found) {
      console.log(`✅ "${practiceName}" found in database (ID: ${found.id})`);
